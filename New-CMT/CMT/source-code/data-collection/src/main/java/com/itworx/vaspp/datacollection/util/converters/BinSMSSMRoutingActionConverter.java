...
public class BinSMSSMRoutingActionConverter extends AbstractTextConverter{
	private Logger logger;
public BinSMSSMRoutingActionConverter()
{}
...
throws ApplicationException, InputException {
	...
	logger
			.debug("Inside BinSMSSMRoutingActionConverter convert - started converting input files");
	String path = PropertyReader.getConvertedFilesPath();
	File[] outputFiles = new File[1];
	File outputFile = new File(path, "SMSSMR"+ inputFiles[0].getName());
	          ...
	          // inputStream = new BufferedReader(new FileReader(inputFiles[i]));
	           ...
	           String line;
	            ...
	            while(inputStream.ready())
	            {
	            	...
	            	else if (line.contains(" sriSmRoutingAction ="))
	            	{
	            		sriRoutingAction = line.split(" sriSmRoutingAction =")[1];
	            		//System.out.println("sriRoutingAction"+sriRoutingAction);
	            		key =date+","+sriRoutingAction;
	...
	catch(IllegalThreadStateException  e)
   {
	   e.printStackTrace();
		...
		logger
				.error("BinSMSSMRoutingActionConverter.convert() - Input file not found "
						+ e);
	...
	logger
			.debug("BinSMSSMRoutingActionConverter.convert() - finished converting input files successfully ");
	return outputFiles;
}
public static void  main(String ag[])
{
	 ...
	 try {
    		...
    		BinSMSSMRoutingActionConverter s = new BinSMSSMRoutingActionConverter();
