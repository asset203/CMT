...
public class BinSMSMOFailureConverter extends AbstractTextConverter{
	private Logger logger;
public BinSMSMOFailureConverter()
{
}
...
throws ApplicationException, InputException {
	...
	logger
			.debug("Inside BinSMSMOFailureConverter convert - started converting input files");
	String path = PropertyReader.getConvertedFilesPath();
	File[] outputFiles = new File[1];
	File outputFile = new File(path,  "SMSMOF"+inputFiles[0].getName());
			   ...
			   String line;
	            ...
	            while(inputStream.ready())
	            {
	            	...
	            	else if (line.contains("trustedMoFwdSmWithCountryAndNetworkInfo"))
	            	{
	            		...
	            		{
	            			if((!line.split(" deliveryResult =")[1].trim().equalsIgnoreCase("success(0)")))
	            			deliverdMSG=line.split(" deliveryResult =")[1].trim();
	            			else
	            				deliverdMSG=null;
	            		}
	            		else
	...
	catch(IllegalThreadStateException  e)
   {
	   e.printStackTrace();
		...
		logger
				.error("BinSMSMOFailureConverter.convert() - Input file not found "
						+ e);
	...
	logger
			.debug("BinSMSMOFailureConverter.convert() - finished converting input files successfully ");
	return outputFiles;
}
public static void  main(String ag[])
{
	 ...
	 try {
    		...
    		BinSMSMOFailureConverter s = new BinSMSMOFailureConverter();
