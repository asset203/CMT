...
public class KPIDailyCountersConverter extends AbstractTextConverter{
	private Logger logger;
	private org.jdom.Document document;
	  private Document xmlDocument;
	public KPIDailyCountersConverter()
	{}
	public File[] convert(File[] inputFiles, String systemName)
	throws ApplicationException {
		logger = Logger.getLogger(systemName);
		logger
				.debug("Inside KPIDailyCountersConverter convert - started converting input files");
	...
	{	
		...
		for (int i = 0; i < inputFiles.length; i++) {
			...
			{ 
				...
				{ 
					Element itemElement= (Element)itemsElement.get(item);
						if(itemElement.getAttributeValue("intervalUnit").equalsIgnoreCase("day"))
						{
			...
			logger.debug("KPIDailyCountersConverter.convert() - finished converting input files successfully ");
	...
	public static void main(String ag[]) {
		try {
			...
			KPIDailyCountersConverter s = new KPIDailyCountersConverter();
