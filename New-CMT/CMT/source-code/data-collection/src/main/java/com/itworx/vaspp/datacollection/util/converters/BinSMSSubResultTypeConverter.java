...
public class BinSMSSubResultTypeConverter  extends AbstractTextConverter{
	private Logger logger;

public BinSMSSubResultTypeConverter()
{
}
...
throws ApplicationException, InputException {
	...
	logger
			.debug("Inside BinSMSSubResultTypeConverter convert - started converting input files");
	String path = PropertyReader.getConvertedFilesPath();
	File[] outputFiles = new File[1];
	File outputFile = new File(path,  "SMSSub"+inputFiles[0].getName());
	         ...
	         //  inputStream = new BufferedReader(new FileReader(inputFiles[i]));
	           ...
	           String line;
	            ...
	            while(inputStream.ready())
	            {
	            	...
	            	else if (line.contains(" submissionResult ="))
	            	{
	            		subResultType = line.split(" submissionResult =")[1];

	            		key =date+","+subResultType;
	...
	catch(IllegalThreadStateException  e)
   {
	   e.printStackTrace();
		...
		logger
				.error("BinSMSSubResultTypeConverter.convert() - Input file not found "
						+ e);
	...
	logger
			.debug("BinSMSSubResultTypeConverter.convert() - finished converting input files successfully ");
	return outputFiles;
}
public static void  main(String ag[])
{
	 ...
	 try {
    		...
    		BinSMSSubResultTypeConverter s = new BinSMSSubResultTypeConverter();
