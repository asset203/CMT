...
public class VodOverTotalDataUsageConverter extends AbstractTextConverter{
	private Logger logger;
	private org.jdom.Document document;
	  private Document xmlDocument;
public VodOverTotalDataUsageConverter()
{
}
...
throws ApplicationException {
	...
	logger
			.debug("Inside VodOverTotalDataUsageConverterconvert - started converting input files");
...
{	
	...
	for (int i = 0; i < inputFiles.length; i++) {
        ...
        for(int r=0;r<mdElements.size();r++)
        {
        	...
        	for(int c=0;c<miElements.size();c++)
        	{
        		...
        		try{	
        		 if (cpl.getAttributeValue("name").equalsIgnoreCase("Total Data Usage in GB"))
        		{
        			
        			 totalDataUsage=Double.parseDouble(cpl.getValue()!=null?cpl.getValue():"0.0");   			 
	...
	logger.debug("VodOverTotalDataUsageConverter.convert() - finished converting input files successfully ");
...
public static void main(String ag[]) {
	try {
		...
		VodOverTotalDataUsageConverter s = new VodOverTotalDataUsageConverter();
		File[] input = new File[1];
		input[0]=new File("D:\\build\\phase10\\DataCollection\\Vodafone Overall 3 Total Data Usage.xml");
