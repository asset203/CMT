/*
 * File:       HLRSubConverter.java
 * Date        Author          Changes
 * 06/20/2006  Nayera Mohamed  Created
 *
 * Converter class for converting HLR subscirbers text input files into comma spearated format
 */
 ...
 */
public class HLRSubConverter extends AbstractTextConverter {
  ...
  private long validSubscribers;
	...
	public File[] convert(File[] inputFiles, String systemName)
		...
		logger
				.debug("HLRSubConverter.convert() - started converting input files ");
		String path = PropertyReader.getConvertedFilesPath();
		File[] outputFiles = new File[1];
		try {
			for (int j = 0; j < inputFiles.length; j++) {
				logger.debug("HLRSubConverter.convert() - converting file "
						+ inputFiles[j].getName());
				BufferedReader inputStream = new BufferedReader(new FileReader(
						inputFiles[j]));
				this.getSubscribers(inputStream,inputFiles[j].getName());      
				inputStream.close();
				logger.debug("HLRSubConverter.convert() - "
						+ inputFiles[j].getName() + " converted");
			}
			File output = new File(path, "HLRSubscribers.txt");
			BufferedWriter outputStream = new BufferedWriter(new FileWriter(
					output));
		      checkCompleteInput();
		      //System.out.println( this.allocatedSubscribers+ "," + this.registeredSubscribers + ","	+ this.validSubscribers+","+Utils.getYesterdaysDate());
			outputStream.write( this.allocatedSubscribers+ "," + this.registeredSubscribers + ","
								+ this.validSubscribers+","+Utils.getYesterdaysDate());
			outputStream.newLine();  
			outputStream.close();
			outputFiles[0] = output;
		} catch (FileNotFoundException e) {
			logger.error("HLRSubConverter.convert() - Input file not found " + e);
			throw new ApplicationException(e);
		} catch (IOException e) {
			throw new ApplicationException(e);
		}
		logger
				.debug("HLRSubConverter.convert() - finished converting input files successfully ");
  		...
  		try {
			...
			if (lowerFileName.indexOf("reg") != -1) {
	          ...
	          this.validSubscribers = this.getNumber(line,fileName);
				...
				logger.error("HLRSubConverter.getSubscribers() - Invalid input file " +e);
        throw new InputException("Invalid input file " + e);
			}
      catch (ArrayIndexOutOfBoundsException e) {
        logger.error("HLRSubConverter.getSubscribers() - Invalid record in input file " +e);
  ...
  private void checkCompleteInput () throws InputException
  {
    ...
    {
      logger.error("HLRSubConverter.getSubscribers() - Incomplete Input ");
  ...
  {
    if (line.equals("")){
			logger.error("HLRSubConverter.getSubscribers()- Invalid input file: "+fileName);
    ...
    {
      logger.error("HLRSubConverter.getSubscribers()- Invalid number in input file: "+fileName);
	...
	public static void main(String ag[]) {
		try {
			...
			HLRSubConverter s = new HLRSubConverter();
