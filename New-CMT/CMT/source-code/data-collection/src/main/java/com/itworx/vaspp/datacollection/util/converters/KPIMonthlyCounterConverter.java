...
public class KPIMonthlyCounterConverter extends AbstractTextConverter{
	private Logger logger;
	private org.jdom.Document document;
	  private Document xmlDocument;
	  public KPIMonthlyCounterConverter()
	  {}
	  public File[] convert(File[] inputFiles, String systemName)
		throws ApplicationException {
			logger = Logger.getLogger(systemName);
			logger
					.debug("Inside KPIMonthlyCounterConverter convert - started converting input files");
		...
		{	
			...
			for (int i = 0; i < inputFiles.length; i++) {
				...
				{ 
					...
					{ 
						Element itemElement= (Element)itemsElement.get(item);
							if(itemElement.getAttributeValue("intervalUnit").equalsIgnoreCase("month"))
							{
				...
				logger.debug("KPIMonthlyCounterConverter.convert() - finished converting input files successfully ");
		...
		public static void main(String ag[]) {
			try {
				...
				KPIMonthlyCounterConverter s = new KPIMonthlyCounterConverter();
