...
public class USSDCIConnectorConverter extends AbstractTextConverter{
	Logger logger;
...
public USSDCIConnectorConverter()
{}
...
throws ApplicationException, InputException {
	...
	logger
			.debug("Inside USSDCIConnectorConverter convert - started converting input files");
	...
	try {
		...
		for (int i = 0; i < inputFiles.length; i++) {
			logger
					.debug("USSDCIConnectorConverter.convert() - converting file "
				...
				if(line.contains(","))
				{
					...
					if(line.contains("New AppRequest"))
					{
						 if(line.contains("]")&&line.contains("["))
						 {
					       ...
					       if(keyVsReqCount.containsKey(date))
					       {
					    	   long count=keyVsReqCount.get(date);
					    	   keyVsReqCount.remove(date);
					    	   keyVsReqCount.put(date, count+1);
					       }
					       else
					       {
					    	   keyVsReqCount.put(date, new Long (1));
					       }
						 }
		...
		while(reqIt.hasNext())
		{
			...
			if(KeyVsRespCount.containsKey(reqKey))
			{
				...
				for(int i=0;i<respList.size();i++)
				{
					...
					outputStream.write(((RespObject)respList.get(i)).getRespKey()+","+((RespObject)respList.get(i)).getRespCount()+","+((RespObject)respList.get(i)).getRespCount());					
					outputStream.newLine();
				}
				if(totalRespCount<reqCount)
				{
					long resultCount=reqCount-totalRespCount;
					outputStream.write(reqKey+", ,"+resultCount+",0");					
					outputStream.newLine();
				}
				
			}else// it means req without respnses 
			{
				outputStream.write(reqKey+", ,"+keyVsReqCount.get(reqKey)+",0");					
				outputStream.newLine();
			}
...
inputStream.close();
		...
		logger.debug("USSDCIConnectorConverter.convert() - finished converting input files successfully ");
	
	}
	catch (FileNotFoundException e) {
			logger
					.error("USSDCIConnectorConverter.convert() - Input file not found "
							+ e);
			throw new ApplicationException(e);
		} catch (IOException e) {
			logger
					.error("USSDCIConnectorConverter.convert() - Couldn't read input file"
							+ e);
			throw new ApplicationException(e);
		}
		logger
				.debug("USSDCIConnectorConverter.convert() - finished converting input files successfully ");
		return outputFiles;
}
public static void main(String ag[]) {
	try {
		...
		USSDCIConnectorConverter s = new USSDCIConnectorConverter();
		File[] input = new File[1];
		input[0]=new File("D:\\build\\phase9\\phase9Builds\\DataCollection\\ipcconnector_2011022812.log");
