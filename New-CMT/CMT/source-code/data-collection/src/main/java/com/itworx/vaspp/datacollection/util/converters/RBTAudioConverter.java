...
public class RBTAudioConverter extends AbstractTextConverter {
	private Logger logger;

	public RBTAudioConverter() {
	}
		...
		try {
			for (int i = 0; i < inputFiles.length; i++) 
			{
				...
				while (inputStream.ready()) 
				{
					...
					outputStream.write(",");
					String second = line.substring(0, line.lastIndexOf(":")).trim();
					String totalCalls = line.substring(line.lastIndexOf(":")+1, line.length()).trim();
					outputStream.write(second);
					outputStream.write(",");
					outputStream.write(totalCalls);
	...
	public static void main(String ag[]) {
		try {
			...
			RBTAudioConverter s = new RBTAudioConverter();
			File[] input = new File[1];
			input[0]=new File("D:\\Audio.RBT");
